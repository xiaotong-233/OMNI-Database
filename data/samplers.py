...
# 设置当前轮次的方法。
# 这个采样器的目的是确保在分布式训练中，每个进程获得其相应的样本子集。如果启用了洗牌，它会根据当前轮次对数据集进行洗牌，以确保每个进程看到不同的样本。
...
# 这个采样器的主要目的是确保在分布式训练中，每个本地进程都能获得其相应的样本子集。如果启用了洗牌，它会根据当前轮次对数据集进行洗牌，以确保每个本地进程看到不同的样本。
