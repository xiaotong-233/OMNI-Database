...
def decodebox(regression, anchors, input_shape):
    ...
    # 裁剪 anchors 使其与 regression 的维度一致
    if anchors.shape[0] > regression.shape[0]:
        anchors = anchors[:regression.shape[0], :]
    #--------------------------------------#
    #   计算先验框的中心
    #--------------------------------------#
...
# 这个函数用于执行非极大抑制 (NMS) 过程，筛选出预测结果中的最佳框。
# 它接受模型的预测结果 (prediction)、输入形状 (input_shape)、图像形状 (image_shape)、letterbox_image 参数、置信度阈值 (conf_thres) 和 NMS 阈值 (nms_thres)。函数返回经过 NMS 处理后的最佳框。
