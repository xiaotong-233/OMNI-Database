...
def preprocess_gt(gt_path, class_names):
    ...
    #             lines_list = file_lines_to_list(os.path.join(root, file))
    #             boxes_per_image = []
    #             image = {}
...
def preprocess_dr(dr_path, class_names):
    results = []
    with open(os.path.join(dr_path, "imageset1", "train.txt")) as train_file:
        train_ids = train_file.read().splitlines()
    with open(os.path.join(dr_path, "imageset1", "test.txt")) as test_file:
        test_ids = test_file.read().splitlines()
    with open(os.path.join(dr_path, "imageset1", "val.txt")) as val_file:
        val_ids = val_file.read().splitlines()

    for root, dirs, files in os.walk(dr_path):
        for folder in dirs:
            image_folder = os.path.join(root, folder, "JPEGImages")
            annotation_folder = os.path.join(root, folder, "Annotations")
            if folder in train_ids:
                image_set = "train"
            elif folder in test_ids:
                image_set = "test"
            elif folder in val_ids:
                image_set = "val"
            else:
                continue
            ...
            for image_id in image_ids:
                lines_list = file_lines_to_list(os.path.join(annotation_folder, image_id))
                image_id = os.path.splitext(image_id)[0]
                for line in lines_list:
                    line_split = line.split()
                    confidence, left, top, right, bottom = line_split[-5:]
