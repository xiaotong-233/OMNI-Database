...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    if load_pretrain_weights:
        ...
        for k in list(weights_dict.keys()):
            if "backbone.fpn" in k:  # 删除所有FPN相关权重
                del weights_dict[k]
            # 删除rpn.head与box_predictor的权重，因为它们的维度不兼容
            if "rpn.head" in k or "roi_heads" in k:
                del weights_dict[k]
            # 删除与mask分支相关的权重
            # if "mask_fcn_logits" in k:
            #     del weights_dict[k]

        # 加载过滤后的权重，strict=False 允许忽略不匹配的层
...
def main(args):
    ...
    # val_dataset = VOCInstances(data_root, transforms=data_transform["val"])
    ...
    optimizer = optim.AdamW(params, lr=0.0005, betas=(0.9, 0.999), weight_decay=0.0005)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        det_info = utils.evaluate(model, val_data_loader, device=device)
        ...
        torch.save(save_files, "./save_weights_all10/model_{}.pth".format(epoch))
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default='/mnt/disk1/data0/jxt/dataset/data/allsides_10/coco', help='dataset')
    # 检测目标类别数(不包含背景)
    parser.add_argument('--num-classes', default=10, type=int, help='num_classes')
    # 文件保存地址
    parser.add_argument('--output-dir', default='./save_weights_allside10', help='path where to save')
