...
# from torch import Tensor
# import torch.nn.functional as F
# from torchvision.ops import roi_align, RoIPool
#
# from . import det_utils
# from . import boxes as box_ops
...
#
#     def check_targets(self, targets):
#         # 验证targets是否为None并包含'boxes'和'labels'
#         assert targets is not None, "Targets should not be None."
#         assert all("boxes" in t for t in targets), "Each target must contain 'boxes'."
#         assert all("labels" in t for t in targets), "Each target must contain 'labels'."
#
#     def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
#         matched_idxs = []
#         labels = []
...
#             matched_idxs = None
#
#         # 如果features是OrderedDict，取出需要的特征图（比如第一个）
#         if isinstance(features, OrderedDict):
#             features = features['0']  # 假设您想使用OrderedDict中的第一个特征图
#
#         # print(f"features type: {type(features)}, shape: {features.shape if isinstance(features, torch.Tensor) else None}")
#         # print(f"proposals type: {type(proposals)}, lengths: {[p.shape for p in proposals]}")
