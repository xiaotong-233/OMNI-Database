...
if __name__ == "__main__":
    ...
    #--------------------------------------------------------------------------------------#
    classes_path    = 'model_data/classes_10.txt'
    #--------------------------------------------------------------------------------------#
    ...
    #   默认指向根目录下的VOC数据集
    #-------------------------------------------------------#
    VOCdevkit_path  = '/mnt/disk1/data0/jxt/dataset/data/allsides_10'
    #-------------------------------------------------------#
    #   结果输出的文件夹，默认为map_out
    #-------------------------------------------------------#
    map_out_path    = 'map_out_all10'

    image_ids = open(os.path.join(VOCdevkit_path, "imageset1/test.txt")).read().strip().split()
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for folders_name in tqdm(image_ids):
            folders_path = os.path.join(VOCdevkit_path, "JPEGImages", folders_name)
            if not os.path.isdir(folders_path):
                continue

            for image_root, _, image_files in os.walk(folders_path):
                for files_name in image_files:
                    # if not files_name.lower().endswith(".JPG"):
                    #     continue
                    images_id = os.path.splitext(files_name)[0]
                    image_paths = os.path.join(image_root, files_name)
                    image = Image.open(image_paths)
                    image_id = "_".join([folders_name, os.path.basename(image_root), images_id])
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for folder_name in tqdm(image_ids):
            folder_path = os.path.join(VOCdevkit_path, "GNN_annotated", folder_name)
            ...
            for root, _, files in os.walk(folder_path):
                for file_name in files:
                    ...
                    xml_id = os.path.splitext(file_name)[0]
                    xml_path = os.path.join(root, file_name)
                    tree = ET.parse(xml_path)
                    root_elem = tree.getroot()
                    name = "_".join([folder_name, os.path.basename(root), xml_id])

                    with open(os.path.join(map_out_path, "ground-truth", name + ".txt"), "w") as new_f:
